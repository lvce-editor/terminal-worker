export const name = 'viewlet.terminal-xterm-multiple-touch'
...
export const test = async ({ Command, expect, KeyBoard, Locator, Settings }) => {
  ...
  await runCommand(terminal, KeyBoard, 'touch a.txt b.txt')
  await runCommand(terminal, KeyBoard, 'ls')

  await expect(terminal).toContainText('a.txt')
  await expect(terminal).toContainText('b.txt')
}
