export const name = 'viewlet.terminal-xterm-ls-sorted'
...
export const test = async ({ Command, expect, KeyBoard, Locator, Settings }) => {
  ...
  await runCommand(terminal, KeyBoard, 'touch zeta.txt alpha.txt middle.txt')
  await runCommand(terminal, KeyBoard, 'ls')

  await expect(terminal).toContainText('alpha.txt\nmiddle.txt\nzeta.txt')
}
